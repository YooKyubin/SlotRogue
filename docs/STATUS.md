...
**맵 Encounter SO** — [`feature-map-encounter-so`](./exec-plans/completed/feature-map-encounter-so.md) **완료** (2026-06-04): encounter·맵 노드/그래프 SO, roster 빌더, formation HUD.

**RunBattle 적 슬롯 프리팹** — [`feature-enemy-formation-slot`](./exec-plans/completed/feature-enemy-formation-slot.md) **완료** (2026-06-04): `EnemyFormationSlot` 프리팹, `MonsterDefinition.portrait`, 슬롯 DamageAnchor + 플로팅 좌표 변환.

**RunBattle formation 2D 월드** — [`feature-enemy-formation-2d-world`](./exec-plans/active/feature-enemy-formation-2d-world.md): UI 슬롯 → 월드 `SpriteRenderer` + World Space HUD; Rebuild가 월드 슬롯 3개까지 생성. 카메라 셰이크·HUD shake 토글은 Later.
...
|------|-------|---------|----------------|
| [`feature-enemy-formation-2d-world.md`](./exec-plans/active/feature-enemy-formation-2d-world.md) | _(미정)_ | 2026-06-05 | RunBattle 몬스터 슬롯 UI→월드 2D, Rebuild 포함 |
