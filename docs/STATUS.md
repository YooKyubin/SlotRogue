...
**다인전 전투 확장** — [`feature-multi-participant-combat`](./exec-plans/active/feature-multi-participant-combat.md): ADR-0004 기준으로 participant id roster, 타겟 선택(`TargetMode` + `TargetParticipantId`), 멀티히트 재타겟, 적 턴 좌→우 순차를 Core/Flow/UI에 반영.
...
|------|-------|---------|----------------|
| [`feature-multi-participant-combat.md`](./exec-plans/active/feature-multi-participant-combat.md) | _(전투 담당)_ | 2026-06-02 | Enemy 다인전 + 타겟 선택 + 멀티히트 재타겟 규칙 도입 |
