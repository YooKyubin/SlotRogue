...
**Monster Battle View 연결** — [`feature-monster-battle-view-integration`](./exec-plans/active/feature-monster-battle-view-integration.md): `MonsterDefinition`의 visual SO에서 combat visual prefab을 선택하고, RunGame 전투 화면의 formation slot `VisualRoot` 아래에 생성한다. Idle/Attack 애니메이션은 기존 Combat Presentation command 흐름에 연결한다.
...
| [`feature-monster-battle-view-integration.md`](./exec-plans/active/feature-monster-battle-view-integration.md) | _(전투/UI 담당)_ | 2026-06-16 | 몬스터 visual prefab을 formation slot에 연결하고 Idle/Attack 애니메이션 경로 구성 |
