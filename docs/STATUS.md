...
**규칙 기반 Encounter 선택** — [`feature-rule-based-encounter-selection`](./exec-plans/active/feature-rule-based-encounter-selection.md): 기존 `MonsterDefinition` 기반 전투 생성 흐름 앞에 EncounterTable/Selector를 추가해 Tier·Cycle·Weight·runSeed 기준으로 몬스터 편성을 결정한다. 1차 범위는 1마리 편성 유지, Dev Override 유지, 기존 roster 생성 경로 재사용이다.
