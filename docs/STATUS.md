...
**맵 Encounter SO** — [`feature-map-encounter-so`](./exec-plans/active/feature-map-encounter-so.md): `RunEncounterDefinition`으로 몬스터 수·종류·배치·(선택) HP/패턴 데이터화. `RunBattleController` roster 빌드만 SO 입력으로 교체; Core·RunBattle HUD/타겟 대개편 없음.
...
|------|-------|---------|----------------|
| [`feature-map-encounter-so.md`](./exec-plans/active/feature-map-encounter-so.md) | _(게임 플로우/전투 데이터)_ | 2026-06-04 | RunEncounterDefinition SO + roster 빌더 + Duo/Trio 검증 |
