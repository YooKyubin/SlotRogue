...
**게임 플로우 루프** — [`feature-game-flow-loop`](./exec-plans/active/feature-game-flow-loop.md): 게임 시작 → 시작 유물 → 전투 → 보상 → 다음 전투로 이어지는 무한모드 루프. 전투 진입은 `CurrentBattleNumber`/`EncounterTier` 기반 `RunEncounterRosterBuilder.BuildForTier()` 단일 경로로 정리했으며, 전투 코드는 수정하지 않고 기존 API로 연결한다. 전투 중 `Relic Inventory Origin`은 심볼 풀 개수와 보유 유물 탭 인벤토리를 연다.
