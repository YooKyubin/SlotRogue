...
**규칙 기반 Encounter 선택** — [`feature-rule-based-encounter-selection`](./exec-plans/active/feature-rule-based-encounter-selection.md): 기존 `MonsterDefinition` 기반 전투 생성 흐름 앞에 EncounterTable/Selector를 추가해 Tier·Cycle·Weight·runSeed 기준으로 몬스터 편성을 결정한다. 현재 `EncounterTable`/`EncounterDefinition`, 선택 결과 타입, 1~3마리 formation layout, 결정적 `EncounterSelector`까지 추가했으며, Dev Override 유지와 기존 roster 생성 경로 재사용은 후속 연결 단계로 남아 있다.
