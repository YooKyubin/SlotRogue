...
**전투 피해 VFX 조합형 모듈** — [`feature-combat-damage-vfx`](./exec-plans/active/feature-combat-damage-vfx.md): 플레이어 직접 피해를 시작으로 피해 종류별 VFX profile을 `MonoBehaviour` module 조합으로 재생하는 presentation 경로를 단계적으로 추가한다. 현재 2-A로 `HitFlashDamageVFXModule`을 추가했으며 다음 단계는 `PlayerDirectDamage` set에 `HitFlash`를 연결하고 Unity에서 확인하는 2-B다.
