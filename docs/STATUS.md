...
**유물 v23 런타임 단일화** — [ADR-0005](./adr/0005-relic-v23-runtime-model.md): v23 80종을 `RelicCatalog`에 등록하고 `OwnedRelics` 전체를 `RelicEffectRunner`로 연결했다. 시작 화면은 S-01~S-06 중 3종을 중복 없이 제시한다. 일반 심볼·태그 조건의 화상·감염·취약·약화·가시 유물은 기존 상태 효과 요청 파이프라인으로 활성화했으며, 별도 실행 시점이나 발동 제한이 필요한 연계 유물은 보상풀에서 제외한다. 몬스터 행동도 같은 `StatusEffectSpec.FromAmount` 계약을 사용하되, 속성 자체를 단일 action effect로 취급해 1 action = 1 effect 구조를 유지한다.
...
| [`feature-tutorial-battle-flow.md`](./exec-plans/completed/feature-tutorial-battle-flow.md) | 2026-06-19 | 2마리 튜토리얼 전투, 인벤토리 확인, 훈련용 배터리, 고정 2스핀, 몬스터 방어 후 다음 공격 의도, 승리 후 로비 복귀 |
