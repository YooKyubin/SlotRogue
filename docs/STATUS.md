...
**규칙 기반 Encounter 선택** — [`feature-rule-based-encounter-selection`](./exec-plans/active/feature-rule-based-encounter-selection.md): 기존 `MonsterDefinition` 기반 전투 생성 흐름 앞에 EncounterTable/Selector를 추가해 ThemeGroup·Tier·Weight·runSeed 기준으로 몬스터 편성을 결정한다. 현재 선택 데이터, formation layout, 결정적 `EncounterSelector`, 인접 theme section 반복 방지, `EncounterSelection` 기반 roster builder, `BattleSceneCompositionRoot` 연결, `WaveScheduleDefinition` 기반 EncounterTier/ThemeSectionIndex 평가, HP 전용 `EncounterScaling`까지 추가했으며, 수동 플레이 검증은 남아 있다.
