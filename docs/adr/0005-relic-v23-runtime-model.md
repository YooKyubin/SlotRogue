...
최신 첨부 기획 `relic_pool_v23_status_balance_patch.html`은 80종 유물과 시작 유물 6종을 기준으로 정의한다. v23의 화상·감염·취약·약화·가시는 기존 전투 코어의 Burn·Poison·Freeze 동작과 동일하지 않으므로, 카탈로그 데이터 반영과 실제 전투 활성화를 분리해야 한다.
...
- **기존 Burn/Poison을 v23 화상/감염으로 간주** — 발동 타이밍과 스택 감소 규칙이 달라 실제 플레이가 기획과 달라지므로 거절한다.
