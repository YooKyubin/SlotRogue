...
| ~~Q2~~ | ~~몬스터 행동 **배열 + 순환 인덱스**~~ | **닫음 (2026-05-31).** `MonsterTurnSchedule` — 턴 세트 `CombatEffect[][]`, 적용 후 index 순환. `UpcomingEnemyActions`/`UpcomingMonsterTurnIndex`. 패턴 SO·RNG는 Later. [`feature-monster-turn-schedule`](../exec-plans/completed/feature-monster-turn-schedule.md) |
