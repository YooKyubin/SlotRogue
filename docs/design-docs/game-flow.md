...
| `RunBattle` | `battle/player-status-panel`, `battle/wave-panel`, `battle/arena`, `battle/formation-slot-1`~`battle/formation-slot-3`, `battle/monster-N-portrait`(SlotId 라벨), `battle/monster-N-status-panel`, `battle/slot-machine-panel`, `battle/slot-cell-00`~`battle/slot-cell-14`, `battle/attack-result-panel`, `battle/spin-button`, `battle/spin-result-panel`, `battle/status-panel`, `battle/energy-panel`, `battle/credits-panel`, `battle/presentation-overlay` |
...
슬롯 셀은 `battle/slot-cell-00`~`battle/slot-cell-14`의 고정 크기 칸으로 배치한다. 몬스터는 Arena 아래 `EnemyFormationSlot` 프리팹 3개(formation HUD)로 표시한다. 초상화는 `RunEncounterEntry.monster` → `MonsterDefinition.portrait`를 슬롯 배열 참조로 `SetPortrait`하며, `GameFlowImageSlot.SlotId`는 에디터 라벨만 쓰고 런타임 검색하지 않는다. portrait 미할당·`monster == null`이면 placeholder 텍스트를 유지한다. 플로팅 데미지 spawn parent는 `battle/presentation-overlay`이고, 위치 기준은 각 슬롯 자식 `DamageAnchor`이다.
