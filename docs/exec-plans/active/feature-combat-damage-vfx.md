...
- [ ] 3-C. slash prefab, spark particle 연결 및 타이밍 확인 — Codex _(보류: particle 아트/튜닝 재개 시)_
...
- [x] `SlashCut`이 Damage VFX Effect Root 위치에서 재생되고 Animation Event 뒤 제거된다.
- [ ] `SparkParticle`이 올바른 몬스터 슬롯 위치에서 재생되고 lifetime 뒤 제거된다.
